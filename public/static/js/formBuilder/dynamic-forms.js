 ...
 */
angular.module('dynform', ['ui.load'])
    .directive('dynamicForm', ['$q', '$parse', '$http', '$templateCache', '$compile', '$document', '$timeout','uiLoad', function ($q, $parse, $http, $templateCache, $compile, $document, $timeout, uiLoad) {
        var supported = {
            //  Text-based elements
            ...
            'cascade': {element: 'div', editable: false, textBased: false},

            'switch': {element: 'div', editable: false, textBased: false},
            'editor': {element: 'textarea', editable: true, textBased: true},
            'upload': {element: 'div', editable:false, textBased: false},
            'tree': {element: 'div', editable:false, textBased: false},
            'kv': {element: 'div', editable: true, textBased: false},
            'tag': {element: 'div', editable: false, textBased: false},
                ...
                if (angular.isDefined(attrs.ngModel) && (angular.isDefined(attrs.template) || angular.isDefined(attrs.templateUrl)) && !element.hasClass('dynamic-form')) {
                    ...
                    (attrs.template ? $q.when($parse(attrs.template)($scope)) :
                        $http.get(attrs.templateUrl, {cache: $templateCache}).then(function (result) {
                            ...
                            var setProperty = function (obj, props, value, lastProp, buildParent) {
                                ...
                                buildFields = function (field, id) {
                                    if (String(id).charAt(0) == '$') {
                                        ...
                                        newElement = angular.element('<button dropzone="dropzoneConfig" class="btn btn-default "><i class="fa fa-plus pull-left"></i>直接将文件拖拽到这里或者点击上传</button>');
                                        newElement = newElement.wrap('<div class="col-sm-10"></div>').parent();
                                        newElement = newElement.wrap('<div class="form-group"></div>').parent();
                                        newElement.prepend(angular.element('<label class="col-sm-2 control-label">'+field.label + '</label>'));
                                        newElement.prepend(angular.element('<input type="hidden" name="'+bracket(field.model) +'" ng-model="'+bracket(field.model, attrs.ngModel) +'"/>'));
                                        this.append(newElement);
                                        newElement = null;
                                    }
                                    //级联菜单
                                        ...
                                        newElement = angular.element('<div class="form-group"></div>');

                                        angular.forEach(field.selects, function (select) {

                                            selectElement = angular.element('<select class="form-control m-l-sm"></select>');
                                            selectElement.attr('name', select.model);
                                            if (angular.isDefined(select.change)){
                                                selectElement.attr('ng-model', select.model);
                                            }else{
                                                selectElement.attr('ng-model', bracket(field.model, attrs.ngModel));
                                            }

                                            if (angular.isDefined(select.change)) {selectElement.attr('ng-change', select.change + "=''");}
                                            if (angular.isDefined(select.autoOptions)) {selectElement.attr('ng-options', select.autoOptions);}

                                            selectElement.prepend(angular.element('<option value="">---请选择---</option>'));
                                            newElement.append(selectElement);
                                            selectElement = null;

                                        });

                                        newElement = newElement.wrap('<div class="col-sm-10 form-inline"></div>').parent();
                                        newElement = newElement.wrap('<div class="form-group"></div>').parent();
                                        newElement.prepend(angular.element('<label class="col-sm-2 control-label">'+field.label + '</label>'));
                                        //newElement.prepend(angular.element('<input type="hidden" name="'+bracket(field.model) +'" ng-model="'+bracket(field.model, attrs.ngModel) +'"/>'));
                                        this.append(newElement);
                                    ...
                                    //键值对
                                        ...
                                        newElement = angular.element('<div class="form-group form-inline"></div>');
                                        angular.forEach(field.options, function (option) {
                                            selectElement = angular.element('<input type="checkbox" ng-model="toggle_'+option.model+'"/>');

                                            selectElement = selectElement.wrap('<span class="input-group-addon"></span>').parent();
                                            selectElement = selectElement.wrap('<div class="input-group"></div>').parent();

                                            selectElement = selectElement.append('<input type="text" class="form-control" ng-model="'+bracket(field.model+'.'+option.model, attrs.ngModel)+'" ng-disabled="!toggle_'+option.model+'">');
                                            selectElement = selectElement.wrap('<div class="col-sm-10 m-b-sm"></div>').parent();
                                            selectElement = selectElement.prepend('<label class="col-sm-2 control-label">'+option.label+'</label>');
                                        ...
                                        newElement = angular.element('<input type="hidden" ui-select2="select2Options" ng-model="'+bracket(field.model, attrs.ngModel)+'" class="form-control w-md">');

                                        newElement = newElement.wrap('<div ui-module="select2"></div>').parent();
                                        ...
                                        newElement = angular.element('<select ng-model="'+bracket(field.model, attrs.ngModel)+'" class="form-control w-md">');

                                        newElement.append(angular.element($document[0].createElement('option')).attr('value', '').attr('selected', 'selected').html(field.empty));
                                        if(angular.isDefined(field.str)) {newElement.append(field.str);}
                                        ...
                                        if (["text","textarea","email","select","password"].indexOf(field.type) > -1) {
                                        ...
                                        if(field.type == "editor"){
                                            newElement.attr('class', 'ueditor');
                                        }

                                        if (!angular.isDefined(field.nullable)) {
                                        ...
                                        if (field.type === 'number' || field.type === 'range') {
                                            ...
                                            if (angular.isDefined(field.options)) {
                                                ...
                                                angular.forEach(field.options, function (option, childId) {
                                                    ...
                                                    if (angular.isDefined(option.label)) {
                                                        newChild = newChild.wrap('<label class="checkbox-inline"></label>').parent();
                                            ...
                                            if (angular.isDefined(field.values)) {
                                                newChild = angular.element('<input type="checkbox" />');
                                                newChild.attr('ng-model', bracket(field.model, attrs.ngModel));
                                                newChild = newChild.wrap('<label class="i-switch m-t-xs m-r"></label>').parent();
                                                newChild.append('<i></i>');
                                                newElement.append(newChild);
                                            }
                                        }
                                            ...
                                            newElement.attr('ui-jq', 'filestyle');
                                        ...
                                        if (angular.isDefined(field.label)) {
                                            ...
                                            //  Button elements get their labels from their contents.
                                                ...
                                                newElement = newElement.attr('class', 'btn btn-success btn-lg');
                                                newElement = newElement.html(field.label);
                                                newElement = newElement.wrap('<div class="col-sm-4 col-sm-offset-2"></div>').parent();
                                                newElement = newElement.wrap('<div class="form-group"></div>').parent();
                                            }
                            ...
                            var addPanel = angular.element('<div class="panel-body"></div>');
